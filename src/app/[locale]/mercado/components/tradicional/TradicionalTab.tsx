  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <Loader2 className="w-8 h-8 animate-spin text-[#00B4C8]" />
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-4 md:p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            <SlidersHorizontal size={16} className="text-[#00B4C8]" />
            {t('filters')}
            {activeFilterCount > 0 && (
              <span className="px-1.5 py-0.5 bg-[#00B4C8] text-white text-[10px] font-bold rounded-full">
                {activeFilterCount}
              </span>
            )}
          </button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-9 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-9 px-2 text-sm border border-gray-200 rounded-lg focus:border-[#00B4C8] focus:outline-none"
              >
        ...
        <div className="mt-3 flex items-center gap-2 text-xs text-gray-500">
          ...
          {activeFilterCount > 0 && (
            <span className="text-[#00B4C8] font-medium">
          ...
          {metrics.avgM2 && (
            <div className="mt-3 text-center">
              <span className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-100 text-sm">
                ...
                <span className="font-bold text-[#00B4C8]">${metrics.avgM2}/m&sup2;</span>
              </span>
            </div>
          )}
      ...
      {hasDevelopments && (
        <div>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-2xl font-bold text-[#1E3A5F]">{t('rankingTitle')}</h2>
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none bg-white border border-gray-200 rounded-lg pl-3 pr-8 py-2 text-sm font-medium text-gray-700 focus:border-[#00B4C8] focus:outline-none cursor-pointer"
              >
                  ...
                  {sortedDevelopments.map((dev, i) => (
                    <tr key={dev.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="text-sm font-semibold text-[#1E3A5F] truncate max-w-[200px]">{dev.name}</div>
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="text-sm font-semibold text-[#00B4C8]">{dev.estimated_rent ? formatPrice(dev.estimated_rent) : '—'}</div>
                      </td>
                      ...
                      <td className="px-3 py-4">
                        <Link href={`/${locale}/desarrollos/${dev.slug}`} className="inline-flex items-center justify-center w-8 h-8 rounded-lg hover:bg-[#00B4C8]/10 text-gray-400 hover:text-[#00B4C8] transition-colors">
        ...
        {buckets.map((count, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
              <div
                className="w-full bg-[#00B4C8]/60 hover:bg-[#00B4C8] rounded-t transition-colors cursor-default"
...
function MetricCard({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-xl p-4 border ${highlight ? 'bg-[#1E3A5F] border-[#1E3A5F] text-white' : 'bg-white border-gray-100'}`}>
      <div className={`text-[10px] font-semibold uppercase tracking-wider mb-1 ${highlight ? 'text-[#00B4C8]' : 'text-gray-400'}`}>{label}</div>
        ...
        {sorted.map(([key, stat]) => (
          <button
            ...
            className={`w-full flex items-center justify-between py-2 px-3 rounded-lg text-left transition-all ${
              activeKey === key
                ? 'bg-[#00B4C8]/10 border border-[#00B4C8]/30'
